...
blackmagicdavinciresolvestudio)
    ...
    appNewVersion=$(curl -fs https://www.blackmagicdesign.com/api/support/us/downloads.json | jq -r '[.downloads.[]|select(.name | startswith ("DaVinci Resolve Studio"))][0].urls|."Mac OS X".[]|"\(.major).\(.minor).\(.releaseNum)"')
