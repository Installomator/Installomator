appsanywhere)
    ...
    seriesPath="$(
      /usr/bin/curl -fsSL "$aaClientReleasePage" 2>/dev/null | /usr/bin/tr -d '\n' | /usr/bin/grep -Eo '<a[^>]+href="[^"]*macos(-client)?-[0-9]+-[0-9]+(-[0-9]+)?"[^>]*>[^<]*macOS[[:space:]]+Client[[:space:]]+[0-9]+\.[0-9]+' | /usr/bin/sed -E 's/.*href="([^"]*)".*/\1/' | /usr/bin/head -n1
    )"
    ...
    latestVersion="$(
      /usr/bin/curl -fsSL "$seriesURL" 2>/dev/null | /usr/bin/sed -nE 's/.*Version[[:space:]]+([0-9]+\.[0-9]+\.[0-9]+).*/\1/p' | /usr/bin/sort -u | /usr/bin/awk -F. '{ printf "%03d%03d%03d %s\n", $1, $2, $3, $0 }' | /usr/bin/sort -r | /usr/bin/awk 'NR==1{ print $2 }'
    )"
