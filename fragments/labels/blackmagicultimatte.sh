blackmagicultimatte)
    ...
    type="pkgInDmgInZip"
    downloadURL=$(curl --compressed --location --header "Content-Type: application/json;charset=UTF-8" --header "User-Agent: Mozilla/5.0" --data '{"country": "us", "platform": "Mac OS X", "product": "Ultimatte"}' \
        ...
        -e "update = parsed.downloads.filter((download) => download.name.match(/^Ultimatte/))[0]" \
